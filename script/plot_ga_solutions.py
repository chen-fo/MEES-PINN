...
def plot_ga_solutions_style(npy_path, start_iter, end_iter, output_dir="solution_space_plots"):
    """
    Plots the first two dimensions of GA solutions with styling referenced from plot_solution_space.py.
    """
    ...
    for i in range(num_points):
        ...
        ax.scatter(x[i], y[i], c=c, s=40, edgecolors='white', linewidth=0.5, zorder=100, label=f"Iter {iter_num}")

    # Add legend to top right with larger, bold font
    legend = ax.legend(loc='upper right', title="Iteration", prop={'size': 12, 'weight': 'bold'})
    ...
    output_filename = f"ga_{start_iter}_{end_iter}.jpg"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Plot GA solution trajectory.")
    ...
    plot_ga_solutions_style(args.npy_file, args.start, args.end, args.output_dir)
