...
def xNES_Adam(f, x0, *, bs=100, lr=1e-2, lr2=0.003, sigma=1e-3, max_iters=10_000,
    ...
    # （不再用 bestFitness 触发保存，但保留变量以兼容返回值）
    bestFitness = -jnp.inf
    bestFound = None
    ...
    for it in range(max_iters):
        ...
        # ✅ 当本次迭代的 loss 更小，就保存“当前中心参数 center”
        if loss_iter < best_loss:
            best_loss = loss_iter
            bestFound = np.array(center)  # 记录该次迭代的中心（用于函数返回）
    ...
    print(f"\nFinished at iter={max_iters}, last loss={loss_ls[-1]:.2e}, best loss={best_loss:.2e}")
    # 为兼容外部变量名，第二个返回值给 -best_loss（相当于“最佳平均fitness”）
    return bestFound, -best_loss, numEvals, iter_time_ls, loss_ls
