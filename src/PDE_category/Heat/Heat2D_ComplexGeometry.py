...
class PDE(VectorizedTask):
    def __init__(self, hidden_layers=None, datapath=ref_dir/'heat_complex.dat', bbox=[-8, 8, -12, 12, 0, 3]):
        ...
        # --- 定义 IC & BC 配置 ---
        # [修改 1] 向量化的辅助函数，返回 shape 为 (N,) 的 boolean array
        def is_on_big_circle(x):
            # 将所有大圆的判断结果堆叠，并在第0维进行“或”运算
            # 如果 x 在任意一个大圆边界上，对应位置为 True
            masks = jnp.stack([circle.on_boundary(x[:, :2]) for circle in big_circles])
            return jnp.any(masks, axis=0)

        def is_on_small_circle(x):
            masks = jnp.stack([circle.on_boundary(x[:, :2]) for circle in small_circles])
            return jnp.any(masks, axis=0)

        # [修改 2] 使用 JAX 兼容的位运算符 (&, ~) 替换 python 的 (and, not)
        # 去掉了 jnp.any(on_boundary)，保证 lambda 返回的是 shape (N,) 的数组
        bc_config = [{
            'component': 0,
            'function': (lambda _: 0),
            ...
            # 修改：使用 & 运算
            'bc': (lambda x, on_boundary: on_boundary & is_on_big_circle(x)),
            'type': 'robin'
        }, {
            'component': 0,
            'function': (lambda _, u: 1 - u),
            # 修改：使用 & 运算
            'bc': (lambda x, on_boundary: on_boundary & is_on_small_circle(x)),
            'type': 'robin'
        }, {
            'component': 0,
            'function': (lambda _, u: 0.1 - u),
            # 修改：使用 & 和 ~ 运算
            'bc': (lambda x, on_boundary: on_boundary & (~is_on_big_circle(x)) & (~is_on_small_circle(x))),
            'type': 'robin'
        }]
