...
class PDE(VectorizedTask):
    def __init__(self, hidden_layers=None, datapath=ref_dir/"poisson_boltzmann2d.dat",
        ...
        def boundary_rec(x, on_boundary):
            return jnp.logical_and(on_boundary, (jnp.isclose(x[:, 0], bbox[0]) | jnp.isclose(x[:, 0], bbox[1]) |
                                                 jnp.isclose(x[:, 1], bbox[2]) | jnp.isclose(x[:, 1], bbox[3]))
                                   )
