...
class multi_SimManager(object):
    ...
    def __init__(self,
        ...
        def step_once(carry, input_data, task):
            ...
            task_state, rewards, done = task.step(task_state, actions)

            accumulated_reward = accumulated_reward + rewards * valid_mask.reshape(-1, 1)
        ...
        def rollout(task_state, policy_states, params, obs_params,
                    step_once_fn, max_steps):
            B = params.shape[0]
            obs = jnp.tile(task_state.obs[None, ...], (B, 1, 1))
            labels = jnp.tile(task_state.labels[None, ...], (B, 1, 1))
            masks = [jnp.tile(m[None, ...], (B, 1)) for m in task_state.bcs_masks]

            task_states = State(obs=obs, labels=labels, bcs_masks=masks)
            accumulated_rewards = jnp.zeros((params.shape[0], 4))
    ...
    def _for_loop_eval(self,
        ...
        scores = jnp.zeros((params.shape[0], 4))
        valid_mask = jnp.ones((params.shape[0], 4))
    ...
    def _scan_loop_eval(self,
        ...
        if self._ma_training:
            ...
            scores = scores.reshape((-1, n_repeats, scores.shape[-1]))
