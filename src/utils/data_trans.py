...
class DataLoader:
    ...
    def _trans_time_data_to_dataset(self, data, datapath, input_dim, output_dim):
        ...
        if times is None or any(t is None for t in times):
            raise ValueError("无法解析 .dat 文件中的时间注释")
        # 只保留每个时间点第一个通道，跳过后续同一时刻的其他通道标记
        times = np.array(times[::output_dim], dtype=np.float32)

        # ——2. 空间维度列数——
        # 这里的 input_dim = 空间维度 + 1（time），所以：
        num_spatial = input_dim - 1

        N = data.shape[0]  # 空间点个数

        # ——3. 坐标扁平化：重复每个点的空间坐标 num_times 次——
        flat_coords = []
        ...
        # ——4. 输出扁平化——
        flat_outputs = []
        for oc in range(output_dim):  # 每个通道
            pieces = []
