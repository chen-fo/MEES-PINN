...
class BasePDE():
    ...
    def trans_time_data_to_dataset(self, datapath):
        ...
        assert slice * self.output_dim == data.shape[
            1] - self.input_dim + 1, "Data shape is not multiple of pde.output_dim"
    ...
    def training_points(self, domain=DEFAULT_NUM_DOMAIN_POINTS, boundary=DEFAULT_NUM_BOUNDARY_POINTS,
                        test=DEFAULT_NUM_TEST_POINTS, mul=1):
    ...
    def check(self):
        ...
        for i in range(self.num_pde):
            if self.loss_config[i]['type'] != 'pde':
                raise ValueError(
                    "All PDE loss should be set before Boundary loss to avoid potential issues with methods like NTK")
