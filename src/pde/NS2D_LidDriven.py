...
class NS2D_LidDriven(VectorizedTask):
    def __init__(self, datapath =ref_dir/'lid_driven_a4.dat', a=4, nu=1 / 100, bbox=[0, 1, 0, 1]):
        # --- 基础设置 ---
        self.max_steps = 1
        ...
        for bc in self.bcs:
            if isinstance(bc, BC):
                geom_mask = self.geom.on_boundary(self.X_candidate)
                mask = bc.on_boundary(self.X_candidate, geom_mask)
            elif isinstance(bc, PointSetBC):
                # 用全 False mask 来跳过 PointSetBC
                mask = jnp.zeros(self.X_candidate.shape[0], dtype=bool)
            else:
                raise TypeError(f"Unknown BC type: {type(bc)}")
            self.bcs_masks.append(mask)
