...
class PINN(BaseNN):
    """
    PINN for Heat2D_Multiscale :
    PDE:
        ∂u/∂t − a·∂²u/∂x² − b·∂²u/∂y² = 0，其中 a, b为常系数

    IC:
        u(x, y, 0) = sin(α·x) · sin(β·y)
...
class Heat2D_Multiscale(VectorizedTask):
    def __init__(self, bbox=[0, 1, 0, 1, 0, 5], pde_coef=(1 / np.square(500 * np.pi), 1 / np.square(np.pi)), init_coef=(20 * np.pi, np.pi)):
        # --- 基础设置 ---
        self.max_steps = 1
        ...
        self.layout = ['u', 'u_x', 'u_y', 'u_t', 'u_xx', 'u_yy']
        ...
        # --- 定义 IC & BC 配置 ---
        def ref_sol(xt):
            return jnp.sin(init_coef[0] * xt[:, 0:1]) * jnp.sin(init_coef[1] * xt[:, 1:2]) * \
                    jnp.exp(-(pde_coef[0] * init_coef[0]**2 + pde_coef[1] * init_coef[1]**2) * xt[:, 2:3])
    ...
    def pde_fn(self, pred, X=None):
        ...
        u_xx, u_yy = pred[:, 4:5], pred[:, 5:6]

        r = u_t - self.pde_coef[0] * u_xx - self.pde_coef[1] * u_yy
