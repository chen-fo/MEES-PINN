...
from src.utils import DataLoader, SimManager, addbc, stack_outputs, multi_SimManager
...
class Burgers1D(VectorizedTask):
    def __init__(self, hidden_layers='10*4', datapath=ref_dir / 'burgers1d.dat', geom=[-1, 1], time=[0, 1], nu=0.01 / np.pi):
        # --- 基础设置 ---
        self.max_steps = 1
...
sim_mgr = SimManager(n_repeats=1, test_n_repeats=1, pop_size=0, n_evaluations=1,
                     ...
                     seed=seeds)
multi_sim_mgr = multi_SimManager(n_repeats=1, test_n_repeats=1, pop_size=0, n_evaluations=1,
                                 policy_net=policy, train_vec_task=train_task, valid_vec_task=train_task,
                                 seed=seeds)
...
def get_multi_fitness(samples):
    scores, _ = multi_sim_mgr.eval_params(params=samples, test=False)
    return scores
