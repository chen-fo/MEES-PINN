...
class PINN(BaseNN):
    """
    PINN for Poisson2D_ManyArea :
    PDE:
        a(x,y)·(∂²u/∂x² + ∂²u/∂y²) + f(x,y) = 0,
        where a(x,y) and f(x,y) are piecewise‐defined on a split×split grid
        (coefficients loaded from 'poisson_a_coef.dat' and 'poisson_f_coef.dat').

    Data Loss (pointset):
        u(x_i, y_i) = u_data(x_i, y_i)   (from reference data loaded via load_ref_data)
...
class Poisson2D_ManyArea(VectorizedTask):
    def __init__(self, datapath =ref_dir/'poisson_manyarea.dat', bbox=[-10, 10, -10, 10], split=(5, 5), freq=2):
        # --- 基础设置 ---
        self.max_steps = 1
        ...
        # 加载 a(x,y), f(x,y) 系数
        self.a_cof = np.loadtxt(ref_dir / 'poisson_a_coef.dat')
        self.f_cof = np.loadtxt(ref_dir / 'poisson_f_coef.dat').reshape(self.split[0], self.split[1], self.freq, self.freq)
    ...
    def get_coef_fn(self, X):
        split_x, split_y = self.split
        freq = self.freq
        bbox = self.bbox

        block_size = jnp.array([
            (bbox[1] - bbox[0] + 2e-5) / split_x,
            (bbox[3] - bbox[2] + 2e-5) / split_y
        ])

        reduced_xy = X - jnp.array([bbox[0], bbox[2]]) + 1e-5
        dom = jnp.floor(reduced_xy / block_size).astype(jnp.int32)
        res = reduced_xy - dom * block_size

        dom_x = jnp.clip(dom[:, 0], 0, split_x - 1)
        dom_y = jnp.clip(dom[:, 1], 0, split_y - 1)

        a = jnp.array(self.a_cof)[dom_x, dom_y][:, None]
        f_table = jnp.array(self.f_cof)

        def f_fn(domx, domy, resxy):
            f_coef = f_table[domx, domy]
            i = jnp.arange(freq).reshape(-1, 1)
            j = jnp.arange(freq).reshape(1, -1)
            sin_i = jnp.sin(jnp.pi * i * (resxy[0] / block_size[0]))
            sin_j = jnp.sin(jnp.pi * j * (resxy[1] / block_size[1]))
            basis = sin_i * sin_j
            const_term = f_coef[0, 0]
            return const_term + jnp.sum(f_coef * basis)

        f = jax.vmap(f_fn)(dom_x, dom_y, res)[:, None]
        return a, f
