...
class PINN(BaseNN):
    """
    PINN for PoissonBoltzmann2D :
    PDE:
        −(∂²u/∂x² + ∂²u/∂y²) + k²·u = f(x,y),
        where
        f(x,y) = A·(μ₀² + x² + μ₁² + y²) · sin(μ₀·π·x) · sin(μ₁·π·y)

    BC (Dirichlet BC):
        u(x,y) = 0.2   on outer rectangular boundary
        u(x,y) = 1.0   on each circular hole boundary
    """
...
class PoissonBoltzmann2D(VectorizedTask):
    def __init__(self, datapath=ref_dir/"poisson_boltzmann2d.dat",
                 k=8, mu=(1, 4), A=10, bbox=[-1, 1, -1, 1],
        ...
        # 域定义
        self.bbox = bbox
        geom = geometry.Rectangle(xmin=[bbox[0], bbox[2]], xmax=[bbox[1], bbox[3]])
        for i in range(len(circ)):
            disk = geometry.Disk(circ[i][0:2], circ[i][2])
        ...
        def boundary_rec(x, on_boundary):
            return jnp.logical_and(on_boundary, (jnp.isclose(x[:, 0], bbox[0]) | np.isclose(x[:, 0], bbox[1]) |
                                                 jnp.isclose(x[:, 1], bbox[2]) | np.isclose(x[:, 1], bbox[3]))
                                   )

        def boundary_circle(x, on_boundary):
            return jnp.logical_and(on_boundary, ~boundary_rec(x, on_boundary))
    ...
    def pde_fn(self, pred, X=None):
        ...
        def f_src(xy):
            x, y = xy[:, 0:1], xy[:, 1:2]
            return self.A * (self.mu[0] ** 2 + x ** 2 + self.mu[1] ** 2 + y ** 2) * jnp.sin(self.mu[0] * jnp.pi * x) * jnp.sin(self.mu[1] * jnp.pi * y)

        r = -(u_xx + u_yy) + self.k**2 * u - f_src(X)
        return r
