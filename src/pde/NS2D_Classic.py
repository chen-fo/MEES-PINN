...
class PINN(BaseNN):
    """
    PINN for Navier-Stokes 2D with Classic Geometry:
    PDE:
...
class NS2D_Classic(VectorizedTask):
    def __init__(self, datapath=ref_dir/'ns2d.dat', nu=1, bbox=[0, 8, 0, 8], circles=[[6, 6, 1.0], [2, 1.0, 0.5]], linear=False):
        # --- 基础设置 ---
        self.max_steps = 1
        ...
        # PDE参数
        self.nu = nu
        self.circles = circles  # circles[]: center0 center1 radius
        self.linear = linear

        # 域定义
        self.bbox = bbox     # bbox: left(x[0]) right(x[0]) bottom(x[1]) top(x[1])
        rec = geometry.Rectangle(xmin=[bbox[0], bbox[2]], xmax=[bbox[1], bbox[3]])
        geom = rec
        disks = []
        for i in range(len(circles)):
            disk = geometry.Disk(circles[i][0:2], circles[i][2])
            disks.append(disk)
            geom = geometry.csg.CSGDifference(geom, disk)
        self.geom = geom
        self.geom_time = None
        ...
        def boundary_up_down(x, on_boundary):
            return jnp.logical_and(
                on_boundary,
                jnp.logical_or(jnp.isclose(x[:, 1], bbox[2]), jnp.isclose(x[:, 1], bbox[3]))
        )

        def boundary_circle(x, on_boundary):
            return jnp.logical_and(
                on_boundary,
                jnp.logical_not(rec.on_boundary(x))  # 前提是 rec.on_boundary(x) 返回 (N,)
            )
        ...
        def data_load(path):
            ...
            X_all = jnp.array(data[:, :2], jnp.float32)  # shape (N, 3)
            y_all = jnp.array(data[:, 2:], jnp.float32)  # shape (N, 2)
