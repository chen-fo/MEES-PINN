...
class Heat2D_VaryingCoef(VectorizedTask):
    def __init__(self, datapath =ref_dir/'heat_darcy.dat', bbox=[0, 1, 0, 1, 0, 5], A=200, m=(1, 5, 1)):
        # --- 基础设置 ---
        self.max_steps = 1
        ...
        def boundary_t0(x, on_initial):
            # 兼容 (3,) 和 (N,3)
            xx = jnp.atleast_2d(x)
            oi = jnp.ravel(on_initial)  # 兼容标量/向量
        ...
        def data_load(path):
            ...
            y_all = jnp.array(data[:, 3:4], jnp.float32)  # shape (N, 2)
        ...
        # --- PDE采样器 (DataSampler) ---
        self.pde_data = DataSampler_T(self.geom_time, self.bcs, mul=4).train_x_all  # 注意：无时间维度，使用 geom 而非 geom_time
        ...
        self.bcs_masks = [jnp.ravel(bc.filter(self.X_candidate)) for bc in self.bcs]
        ...
        def reset_fn(key):
            ...
            masks = [jnp.squeeze(bc.filter(X_eq)) for bc in self.bcs]  # 去掉多余维度
    ...
    def _prepare_coef_grid(self):
        ...
        yg = np.unique(coef[:, 1])
        nx, ny = len(xg), len(yg)
        idx = np.lexsort((coef[:, 1], coef[:, 0]))  # 先按 x 后按 y
    ...
    def _a_nearest(self, x, y):
        """
        纯 JAX 最近邻查表，避免 SciPy。在 vmap/scan 里安全。
        x,y: (N,1) -> 返回 a(x,y): (N,1)
        """
    ...
    def pde_fn(self, pred):
        # pred: (N, 4) -> [u, u_t, u_xx, u_yy]
        # 坐标来自 loss_fn 里缓存的当前 PDE 批次
        ...
        # 纯 JAX 最近邻查表，替代 scipy.interpolate.griddata
    ...
    def loss_fn(self, pred, X_batch, Y_batch, bcs_masks):
        ...
        # ---- PDE loss ----
        self._current_X_pde = X_pde           # 缓存坐标，供 pde_fn 使用
