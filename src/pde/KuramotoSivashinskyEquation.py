...
class KuramotoSivashinskyEquation(VectorizedTask):
    def __init__(self, datapath =ref_dir/'Kuramoto_Sivashinsky.dat', bbox=[0, 2 * np.pi, 0, 1], alpha=100 / 16, beta=100 / (16 * 16), gamma=100 / (16**4)):
        # --- 基础设置 ---
        self.max_steps = 1
        ...
        def data_load(path):
            ...
            # t_min, t_max = self.bbox[2], self.bbox[3]

            # # 只根据 x 和 t 范围过滤，去掉对 y 的引用
            # mask = (
            #         (X_all[:, 0] >= x_min) & (X_all[:, 0] <= x_max) &  # x 范围
            #         (X_all[:, 1] >= t_min) & (X_all[:, 1] <= t_max)    # t 范围
            # )
            return X_all, y_all
    ...
    def pde_fn(self, pred):
        """根据网络输出action计算 pde 残差"""
        
        # 从 pred 中提取 u, u_t, u_x, u_xx, u_xxxx
...
seeds = 0
train_task = KuramotoSivashinskyEquation()
