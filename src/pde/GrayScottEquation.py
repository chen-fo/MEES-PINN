...
class GrayScottEquation(VectorizedTask):
    def __init__(self, BatchSize_eq=8192, BatchSize_data=4096, datapath =ref_dir / 'grayscott.dat', bbox=[-1, 1, -1, 1, 0, 200], b=0.04, d=0.1, epsilon=(1e-5, 5e-6), ):
        # --- 基础设置 ---
        self.max_steps = 1
        ...
        # 设置扩散系数
        self.epsilon_u = epsilon[0]  # u的扩散系数
        self.epsilon_v = epsilon[1]  # v的扩散系数
    ...
    def pde_fn(self, pred):
        ...
        u, v = pred[:, 0:1], pred[:, 1:2]  # u 和 v 是模型的预测值
        u_t, u_xx, u_yy = pred[:, 2:3], pred[:, 3:4], pred[:, 4:5]  # 只取 u_t, u_xx, u_yy
        v_t, v_xx, v_yy = pred[:, 5:6], pred[:, 6:7], pred[:, 7:8]  # 只取 v_t, v_xx, v_yy
