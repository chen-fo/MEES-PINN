...
class NS2D_BackStep(VectorizedTask):
    def __init__(self, datapath=ref_dir/'ns_0_obstacle.dat',nu=1 / 100,
        bbox=[0, 4, 0, 2],
        # obstacle={
        ...
        def data_load(path):
            ...
            X_all = jnp.array(data[:, :2], jnp.float32)  # 仅保留空间维度，假设数据的前两列是空间坐标
            y_all = jnp.array(data[:, 2:], jnp.float32)  # 假设y的数据从第三列开始
#             # 根据定义域掩码对数据进行过滤
#             x_min, x_max = self.bbox[0], self.bbox[1]
#             y_min, y_max = self.bbox[2], self.bbox[3]

#             mask = (
#                 (X_all[:, 0] >= x_min) & (X_all[:, 0] <= x_max) &  # 过滤x坐标
#                 (X_all[:, 1] >= y_min) & (X_all[:, 1] <= y_max)    # 过滤y坐标
# )
            return X_all, y_all
        ...
        # --- PDE采样器 (DataSampler) ---
        self.pde_data = DataSampler(self.geom, self.bcs, mul=4).train_x_all  # 注意：无时间维度，使用 geom 而非 geom_time
        ...
        # 确保 `self.bcs_masks` 是布尔数组，并具有正确的维度
        self.bcs_masks = [jnp.array(bc.filter(self.X_candidate), dtype=jnp.bool_) for bc in self.bcs]
        # 确保索引时使用的是布尔数组
        self.bcs_points = [X_ref[mask] for mask in self.bcs_masks if mask.shape == X_ref.shape[0:1]]
    ...
    def loss_fn(self, pred, X_batch, Y_batch, bcs_masks):
        ...
        # ---- PDE loss ----
        r_all = self.pde_fn(pde_pred)       # 现在是 (N_pde, 3) 了
        ...
        for bc, mask in zip(self.bcs, bcs_masks):
            ...
            err = bc.error(pde_pred, X_pde)  # 注意也要用 pde_pred(二维)
        ...
        # ---- Data loss ----
        data_loss = self.data_fn(Y_batch, pred2d, interior_mask)  # 这里也用二维的 pred
        ...
        jax.debug.print("pde={:}, ic={:}, bc={:}, data={:}", pde_loss, ic_loss, bc_loss, data_loss)
        return total_loss
...
class PINNsPolicy(PolicyNetwork):
    ...
    def get_actions(self,
        ...
        obs = t_states.obs
        if obs.ndim == 2:
            # (N_pts, dim) → (1, N_pts, dim)
            obs = obs[None, ...]
        obs_shared = obs[0]  # (N_pts, dim)，所有个体共用

        # forward + derivatives
        def f_single(params):
            # 对每个点调用 derivatives，而不是一次性传 N_pts
            def per_point(x):
                return stack_outputs(self.net.derivatives(params, x[None, :]), self.grad_keys)  # (1, layout_dim)
            return jax.vmap(per_point)(obs_shared)  # (N_pts, layout_dim)

        # 只对参数做 vmap
        actions = jax.vmap(f_single)(params_tree)            # (B, N_pts, 4)
        return actions, p_states


# # Initialize task & policy
seeds = 0
train_task = NS2D_BackStep()
