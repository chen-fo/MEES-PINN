...
class PoissonInv(VectorizedTask):
    def __init__(self, bbox=[0, 1, 0, 1]):
        # --- 基础设置 ---
        self.max_steps = 1
        ...
        # --- 定义 IC & BC 配置 ---
        def ref_sol(xy):
            return jnp.concatenate((self.u_ref(xy), self.a_ref(xy)), axis=1)
        ...
        # --- data points ---
        X_data = X_pde  # 由于没有参考数据集，因此不妨令Data点就是pde的内点
