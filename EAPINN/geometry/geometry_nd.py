...
class Hypercube(Geometry):
    ...
    def inside(self, x):
        return jnp.logical_and(
            jnp.all(x >= self.xmin, axis=-1), jnp.all(x <= self.xmax, axis=-1)
        )

    def on_boundary(self, x):
        _on_boundary = jnp.logical_or(
            jnp.any(jnp.isclose(x, self.xmin), axis=-1),
            jnp.any(jnp.isclose(x, self.xmax), axis=-1),
        )
        return jnp.logical_and(self.inside(x), _on_boundary)


    def boundary_normal(self, x):
        _n = -jnp.isclose(x, self.xmin).astype(config.real(jnp)) + \
            jnp.isclose(x, self.xmax).astype(config.real(jnp))

        idx = jnp.count_nonzero(_n, axis=-1) > 1

        # 计算所有范数，避免布尔索引
        l_all = jnp.linalg.norm(_n, axis=-1, keepdims=True)

        # 如果 idx 为真，则归一化；否则保留原值
        l_safe = jnp.where(idx[:, None], l_all, jnp.ones_like(l_all))
        _n = _n / l_safe
...
class Hypersphere(Geometry):
    ...
    def on_boundary(self, x):
        return jnp.isclose(jnp.linalg.norm(x - self.center, axis=-1), self.radius)
    ...
    def boundary_normal(self, x):
        ...
        _n = _n / l * jnp.isclose(l, self.radius)
        return _n

    def random_points(self, n, random="pseudo"):
        # https://math.stackexchange.com/questions/87230/picking-random-points-in-the-volume-of-sphere-with-uniform-probability
        if random == "pseudo":
            U = np.random.rand(n, 1).astype(config.real(jnp))
            X = np.random.normal(size=(n, self.dim)).astype(config.real(jnp))
        else:
            rng = sample(n, self.dim + 1, random)
            U, X = rng[:, 0:1], rng[:, 1:]  # Error if X = [0, 0, ...]
            X = stats.norm.ppf(X).astype(config.real(jnp))
