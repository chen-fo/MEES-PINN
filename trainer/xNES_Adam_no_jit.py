"""
xNES-Adam implementation without JAX JIT compilation.
...
def train(f, policy, sim_mgr, *, bs=100, lr=2e-2, lr2=0.003, sigma=1e-3, max_iters=10_000,
    ...
    history_center = [np.array(center)]
    history_A = [np.array(A)]
    ...
    for it in range(max_iters):
        ...
        # Record history
        history_center.append(np.array(center))
        history_A.append(np.array(A))
    ...
    # Save history
    save_path = "xNES_matrix_history.npz"
    np.savez(save_path, center=np.array(history_center), A=np.array(history_A))
    print(f"Recorded center and A matrices history to {os.path.abspath(save_path)}")
